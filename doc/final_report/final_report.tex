...
\maketitle
\begin{abstract}
In this paper, we present \LS, an Android app that tracks and identifies daily activity of a handheld device user.
\LS harnesses input from a variety of sensors to detect what activity is being performed by the user. Detected activities can then summarized on a time and geographic scale chosen by the user.
...
The modern smartphone provides the possibility to connect to a wide array of sensors, some of them onboard, some other standalone. This ultimately enables sensor-rich computing on the phone, which in turns opens up new possibility of interaction with the user. In particular, through sensor-fusion the smartphone can act as a gateway to monitor the user daily activity.
These increased monitoring capabilities have unlocked the possibility of ``self knowledge through numbers'', to put it like the recently founded movement Quantified Self~\cite{quantified:self}. This means applying analytics to your daily life to gain knowledge about your life, with the goal of ultimately improve it.
However, the Quantified Self is not the end of the story, rather, it is the beginning. After detecting activities, and analyzing them, smartphones provide a gateway to perform actions in response to activities. This means closing the loop and have the smartphone to affect our daily activity performing actions (e.g., send emails, texts, ring, etc.) in response to the recognized activities.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Related Work}
There has been an increasing interest in the pervasive computing community about "context monitoring" and "context recognition" for mobile devices. Context recognition applications are able to identify a person's activity at a particular point in time~\cite{fahy2004cass}. A recent development of the context recognition is the idea of context monitoring~\cite{lee2012mobicon} which involves execution of complex multi-step operations and correlating the output of different sensors.

While the research community has mainly focused on defining an middleware to provide context information to application layer in order to centralize and coordinate access to the shared sensors,  such systems are usually experimental and not yet freely available.
An exception to the rule is The {\tt funf} Open Sensing Framework~\cite{funf}  of MIT Media Lab, freely available and ready to use. Funf provides a common layer abstraction on the device sensors. The management of sensors is centralized and that allows multiple applications to interact with the same sensor without causing conflict or dramatically increase usage.

To conclude the review of related work, we discuss {\tt on\{X\}} a recent product developed at Microsoft~\cite{onx}. {\tt on\{X\}} provides a framework to program actions to be executed automatically by a smartphone when conditions on the sensors are met. Example are ``Launch the music app when I am walking'' or ``Launch calendar when I arrive at work''. 
Microsoft {\tt on\{X\}} can be viewed as the evolution of early apps that would change basic behavior of the phone based on the context, one example being Android Locale~\cite{android:locale}, which mutes the phone depending on location. 
...
In general, \LS works as follows:
\begin{enumerate}
\item When a transition in location is detected (departure from or arrival at a place) events corresponding to sensor readings start being collected;
\item events collected since the previous location transition are analyzed to identify actions, and then discarded. 
\end {enumerate}
...
\subsection{Future Work} As far as the future work is concerned, we recognize that a system like \LS would benefit of many additional features if it were to be released to the general public. 
